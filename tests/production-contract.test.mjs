...
test('recommendation weights are transparent and sum to 1',()=>{const s=read('lib/services/recommendations/score.ts');const m=s.match(/const weights = \{([^}]+)\}/s);assert.ok(m);const values=[...m[1].matchAll(/:([0-9.]+)/g)].map(x=>Number(x[1]));assert.equal(values.length,8);assert.equal(values.reduce((a,b)=>a+b,0),1);assert.match(s,/const conversion = \.5/);});
