...
create table if not exists public.tool_snapshots(
 ...
 source_url text not null, content_hash text not null, title text, description text, pricing jsonb,
...
create table if not exists public.ai_jobs(
 id uuid primary key default gen_random_uuid(), type text not null, status text not null default 'queued',
 input jsonb not null default '{}', output jsonb, error text, created_by uuid references auth.users(id) on delete set null,
 created_at timestamptz not null default now(), started_at timestamptz, completed_at timestamptz
);
create index if not exists ai_jobs_status_idx on public.ai_jobs(status,created_at desc);

create table if not exists public.conversions(
 id uuid primary key default gen_random_uuid(), affiliate_click_id uuid references public.affiliate_clicks(id) on delete set null,
 tool_id uuid references public.tools(id) on delete set null, external_reference text, amount numeric(12,2), currency char(3),
...
begin
 if exists(select 1 from public.reviews r where r.tool_id=new.tool_id and r.user_id=new.user_id and r.id<>coalesce(new.id,'00000000-0000-0000-0000-000000000000'::uuid) and r.status in ('pending','approved','flagged')) then raise exception 'You already reviewed this tool'; end if;
 return new;
end; $$;
drop trigger if exists review_duplicate_guard on public.reviews;
create trigger review_duplicate_guard before insert or update on public.reviews for each row execute function public.prevent_review_duplicates();

create or replace function public.refresh_tool_rating() returns trigger language plpgsql as $$
begin
 update public.tools t set rating=x.avg_rating, review_count=x.review_count, updated_at=now()
 from (select tool_id, avg(rating)::numeric(3,2) avg_rating, count(*)::int review_count from public.reviews where tool_id=coalesce(new.tool_id,old.tool_id) and status='approved' group by tool_id) x
 where t.id=x.tool_id;
 return coalesce(new,old);
end; $$;
drop trigger if exists refresh_tool_rating_trigger on public.reviews;
create trigger refresh_tool_rating_trigger after insert or update or delete on public.reviews for each row execute function public.refresh_tool_rating();
