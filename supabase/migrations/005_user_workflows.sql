...
create policy "submitters create submissions" on public.tool_submissions for insert to authenticated with check(submitter_id=auth.uid());
create policy "submitters read submissions" on public.tool_submissions for select to authenticated using(submitter_id=auth.uid() or public.is_editor_or_admin());
...
create policy "published collections" on public.collections for select using(status='published' or owner_id=auth.uid() or public.is_editor_or_admin());
create policy "owners manage collections" on public.collections for all to authenticated using(owner_id=auth.uid()) with check(owner_id=auth.uid());
...
create policy "published collection tools" on public.collection_tools for select using(exists(select 1 from public.collections c where c.id=collection_id and (c.status='published' or c.owner_id=auth.uid() or public.is_editor_or_admin())));
create policy "owners manage collection tools" on public.collection_tools for all to authenticated using(exists(select 1 from public.collections c where c.id=collection_id and c.owner_id=auth.uid())) with check(exists(select 1 from public.collections c where c.id=collection_id and c.owner_id=auth.uid()));
...
create policy "reviews authenticated insert" on public.reviews for insert to authenticated with check(auth.uid()=user_id and status='pending');
