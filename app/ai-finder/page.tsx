...
import { ArrowRight, CheckCircle2, ExternalLink, GitCompareArrows, Search, Sparkles, WandSparkles } from 'lucide-react'
...
function ToolLogo({ tool }: { tool: any }) {
  return tool.logo_url ? <img src={tool.logo_url} alt={`${tool.name} logo`} width={48} height={48} loading="lazy" style={{ width: 48, height: 48, objectFit: 'contain', borderRadius: 12 }} /> : <div className="tool-logo" style={{ width: 48, height: 48, margin: 0 }}><Sparkles size={20} /></div>
}

function ToolMeta({ tool, copy }: { tool: any; copy: any }) {
  return <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', fontSize: 13 }}>
    {tool.rating != null && <span className="badge">★ {tool.rating}{tool.review_count ? ` (${tool.review_count})` : ''}</span>}
    {tool.pricing_type && <span className="badge">{tool.pricing_type}</span>}
    {tool.starting_price != null && <span className="badge">{tool.currency || '$'}{tool.starting_price}+</span>}
    {tool.verified && <span className="badge"><CheckCircle2 size={13} /> {copy.verified}</span>}
    {tool.affiliate_enabled && <span className="badge">Affiliate</span>}
  </div>
}
...
export default function FinderPage() {
  ...
  return <main className="container" dir={isArabic ? 'rtl' : 'ltr'} style={{ padding: '60px 0 100px' }}>
    ...
    {data && <section style={{ marginTop: 36 }}>
      ...
      {data.aiExplanation && <section className="card" style={{ padding: 24, marginBottom: 24, borderColor: '#6d5dd3' }}><div className="eyebrow">{isArabic ? 'لماذا هذه التوصية؟' : 'Why Eldevo recommends this'}</div><div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 8 }}><Sparkles size={20} /><strong style={{ fontSize: 19 }}>{data.aiExplanation.summary}</strong></div>{data.aiExplanation.reasons?.length > 0 && <ul style={{ margin: '14px 0 0', paddingLeft: isArabic ? 0 : 20, paddingRight: isArabic ? 20 : 0 }}>{data.aiExplanation.reasons.map((reason: string) => <li key={reason} style={{ marginBottom: 7 }}>{reason}</li>)}</ul>}</section>}
      ...
      {data.stack?.length > 0 && <section style={{ marginTop: 34 }}><div className="section-head"><div><div className="eyebrow">{copy.stackEyebrow}</div><h2>{copy.stackTitle}</h2><p className="muted" style={{ margin: 0, maxWidth: 720 }}>{copy.stackDescription}</p></div></div><div className="tool-grid">{data.stack.map((item: any) => <article className="card tool-card" key={`${item.key}-${item.tool.id}`}><div style={{ display: 'flex', gap: 12, alignItems: 'center' }}><ToolLogo tool={item.tool} /><div><div className="eyebrow">{copy.stackStep} · {item.label}</div><h3 style={{ margin: '6px 0' }}>{item.tool.name}</h3></div></div><p className="muted">{item.why}</p><ToolMeta tool={item.tool} copy={copy}/><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 10 }}><span className="badge">{item.score}% match</span>{item.tool.affiliate_enabled && <span className="badge">{isArabic ? 'رابط تابع' : 'Affiliate'}</span>}</div><div style={{ marginTop: 'auto', paddingTop: 12 }}><Link className="btn btn-primary" href={`/go/${item.tool.slug}`}>{copy.stackOpen} <ExternalLink size={14} /></Link></div></article>)}</div></section>}
      {(data.results || []).map((group: any) => <div key={group.key} style={{ marginTop: 32 }}><div className="section-head"><h2>{copy[groupLabels[group.key as keyof typeof groupLabels]]}</h2><span className="muted">{group.items.length} {group.items.length === 1 ? copy.match : copy.matches}</span></div><div className="tool-grid">{group.items.map((recommendation: any) => <article className="card tool-card" key={`${group.key}-${recommendation.tool.id}`}><div style={{ display: 'flex', gap: 12, alignItems: 'center' }}><ToolLogo tool={recommendation.tool} /><div style={{ flex: 1 }}><div className="eyebrow">{recommendation.tool.category?.name || copy.genericTool}</div><h3 style={{ fontSize: 22, margin: '4px 0' }}>{recommendation.tool.name}</h3></div><strong style={{ fontSize: 22, color: '#a79cff' }}>{recommendation.score}%</strong></div><p className="muted">{recommendation.tool.short_description}</p><ToolMeta tool={recommendation.tool} copy={copy}/><h4 style={{ margin: '18px 0 6px' }}>{isArabic ? 'لماذا يناسبك؟' : 'Why it matches you'}</h4>{recommendation.why?.length ? <ul style={{ paddingLeft: isArabic ? 0 : 20, paddingRight: isArabic ? 20 : 0, marginTop: 0 }}>{recommendation.why.map((reason: string) => <li key={reason}>{reason}</li>)}</ul> : <p className="muted">{isArabic ? 'تطابق قوي بناءً على طلبك.' : 'Strong match based on your request.'}</p>}{recommendation.limitations?.length > 0 && <p className="muted" style={{ fontSize: 13 }}>{copy.limitations} {recommendation.limitations.join(', ')}</p>}<div style={{ display: 'flex', gap: 8, marginTop: 'auto', paddingTop: 10, flexWrap: 'wrap' }}><Link className="btn btn-primary" href={`/go/${recommendation.tool.slug}`}>{copy.visit} <ArrowRight size={14} /></Link><Link className="btn btn-secondary" href={`/tools/${recommendation.tool.slug}`}>{copy.details} <Search size={14} /></Link><Link className="btn btn-secondary" href={`/compare/${recommendation.tool.slug}`}><GitCompareArrows size={14} /> {copy.compare}</Link></div></article>)}</div></div>)}
    </section>}
  </main>
}
