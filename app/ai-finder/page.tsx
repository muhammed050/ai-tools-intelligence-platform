...
export default function FinderPage() {
  ...
  return <main className="container" dir={isArabic ? 'rtl' : 'ltr'} style={{ padding: '60px 0 100px' }}>
    ...
    {data && <section style={{ marginTop: 36 }}>
      ...
      {data.aiExplanation && <section className="card" style={{ padding: 24, marginBottom: 24, borderColor: '#6d5dd3' }}>
        <div className="eyebrow">{isArabic ? 'لماذا هذه التوصية؟' : 'Why Eldevo recommends this'}</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 8 }}><Sparkles size={20} /><strong style={{ fontSize: 19 }}>{data.aiExplanation.summary}</strong></div>
        {data.aiExplanation.reasons?.length > 0 && <ul style={{ margin: '14px 0 0', paddingLeft: isArabic ? 0 : 20, paddingRight: isArabic ? 20 : 0 }}>{data.aiExplanation.reasons.map((reason: string) => <li key={reason} style={{ marginBottom: 7 }}>{reason}</li>)}</ul>}
      </section>}
