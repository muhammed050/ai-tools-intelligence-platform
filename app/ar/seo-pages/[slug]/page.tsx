...
export function generateStaticParams() {
  return seoPages.filter((p) => p.slug !== 'best-ai-tools').map((p) => ({ slug: p.slug }))
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const ar = `${site}/ar/seo-pages/${page.slug}`
  const en = `${site}/seo-pages/${page.slug}`
  const title = `أفضل ${page.title.replace(/^Best /, '')} | Eldevo`
  return {
    title,
    description: page.description,
    alternates: { canonical: ar, languages: { ar, en, 'x-default': en } },
    openGraph: { type: 'article', title, description: page.description, url: ar, locale: 'ar_SA', alternateLocale: ['en_US'] },
...
export default async function ArabicSeoPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="container" dir="rtl" style={{ padding: '58px 0 100px' }}>
    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(collectionSchema) }} />
    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faqSchema) }} />
    <nav className="muted" aria-label="مسار التنقل"><Link href="/ar">Eldevo</Link> / <Link href="/ar/tools">أدوات الذكاء الاصطناعي</Link> / {page.title}</nav>
    <section className="hero-glow card" style={{ padding: '48px 28px', textAlign: 'center', maxWidth: 980, margin: '16px auto 0' }}>
      ...
      <h1 style={{ fontSize: 'clamp(40px,6vw,64px)', lineHeight: 1, letterSpacing: '-.05em' }}>{page.title.replace(/^Best /, 'أفضل ')}</h1>
      <p className="muted" style={{ fontSize: 18, maxWidth: 720, margin: '18px auto 25px' }}>{page.description}</p>
      <Link className="btn btn-primary" href={`/ar/ai-finder?q=${encodeURIComponent(page.query)}`}>اعثر على أفضل خيار لي <ArrowRight size={15} /></Link>
    </section>
    <section style={{ marginTop: 45 }}>
      <div className="section-head"><div><div className="eyebrow">ترشيحات Eldevo</div><h2>ابدأ بالأدوات ثم اختر حسب احتياجاتك</h2></div><Link className="btn btn-secondary" href="/ar/tools">تصفح كل الأدوات <ArrowRight size={15}/></Link></div>
      <div className="tool-grid">{tools.map((tool: any) => <article className="card tool-card" key={tool.id} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>{tool.logo_url ? <img src={tool.logo_url} alt={`${tool.name_ar || tool.name} logo`} width={48} height={48} loading="lazy" style={{ borderRadius: 12, objectFit: 'cover' }} /> : <div className="tool-logo">{(tool.name_ar || tool.name).slice(0, 1)}</div>}<h3 style={{ margin: 0 }}>{tool.name_ar || tool.name}</h3><p className="muted" style={{ margin: 0 }}>{tool.short_description_ar || tool.short_description}</p><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', fontSize: 13 }}><span>★ {tool.rating ?? '—'} ({tool.review_count ?? 0})</span>{tool.verified && <span className="badge"><CheckCircle2 size={12}/> موثّق</span>}<span>{tool.pricing_type || 'السعر يختلف'}</span></div><Link style={{ marginTop: 'auto' }} href={`/ar/tools/${tool.slug}`}>عرض المراجعة والتفاصيل <ArrowRight size={13}/></Link></article>)}</div>
    </section>
    <section className="card" style={{ padding: 26, marginTop: 34 }}><h2>اختر حسب احتياجك الحقيقي</h2><p className="muted">Eldevo لا يفترض أن أداة واحدة هي الأفضل للجميع. اكتب مهمتك وميزانيتك والميزات والمنصة المطلوبة، وسيقوم Finder بترتيب الخيارات المتاحة حسب متطلباتك.</p><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 14 }}><Link className="btn btn-secondary" href={`/ar/ai-finder?q=${encodeURIComponent(page.query)}`}>استخدم Finder</Link><Link className="btn btn-secondary" href="/ar/compare">قارن الأدوات</Link></div></section>
    <section style={{ marginTop: 34 }}><h2>الأسئلة الشائعة</h2>{faq.map((item) => <details className="card" style={{ padding: 18, marginTop: 10 }} key={item.q}><summary style={{ fontWeight: 700, cursor: 'pointer' }}>{item.q}</summary><p className="muted" style={{ marginBottom: 0 }}>{item.a}</p></details>)}</section>
  </main>
}
