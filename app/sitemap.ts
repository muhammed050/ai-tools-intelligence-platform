...
function localizedUrl(base: string, path: string, locale: 'en' | 'ar') {
  return locale === 'en' ? `${base}${path || '/'}` : `${base}/ar${path || '/'}`
}
...
function seoPath(slug: string) { return `/seo-pages/${slug}` }
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    const [toolsResult, categoriesResult, articlesResult] = await Promise.all([
      db.from('tools').select('slug,updated_at').eq('status', 'published').order('updated_at', { ascending: false }).range(0, MAX_URLS - 1),
      db.from('categories').select('slug,updated_at').order('name').range(0, 5000),
      db.from('articles').select('slug,updated_at,published_at').eq('status', 'published').order('published_at', { ascending: false }).range(0, 5000),
    ])
    ...
    try {
      ...
      for (const seo of seoPages) {
        if (seo.slug === 'best-ai-tools') continue
        entries.push(localizedEntry(base, seoPath(seo.slug), now, 0.75, 'weekly'))
      }
    } catch {
      // Keep the primary sitemap available if the SEO catalog cannot be loaded.
    }

    for (const item of categoriesResult.data ?? []) entries.push(localizedEntry(base, `/categories/${item.slug}`, item.updated_at ? new Date(item.updated_at) : now, 0.7))
    for (const item of articlesResult.data ?? []) entries.push(localizedEntry(base, `/blog/${item.slug}`, item.updated_at ? new Date(item.updated_at) : item.published_at ? new Date(item.published_at) : now, 0.6, 'monthly'))
    for (const item of toolsResult.data ?? []) entries.push(localizedEntry(base, `/tools/${item.slug}`, item.updated_at ? new Date(item.updated_at) : now, 0.8))
