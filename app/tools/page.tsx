...
export default async function Tools({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  try {
    ...
    const [categoryResult, toolResult] = await Promise.all([
      ...
      (async () => {
        let query = db.from('tools').select('id,name,name_ar,slug,short_description,short_description_ar,rating,review_count,pricing_type,logo_url,verified,featured,platforms,official_url,affiliate_url,affiliate_enabled,affiliate_program_url,category:categories!inner(name,name_ar,slug)').eq('status', 'published')
