...
async function getContext() { try { const db = await createClient(); const { data: { user } } = await db.auth.getUser(); return { db, user } } catch (error) { console.error('Favorites configuration unavailable', error); return { db: null, user: null } } }

export async function GET(request: Request) {
  const { db, user } = await getContext()
  if (!db) return NextResponse.json({ error: 'Favorites are temporarily unavailable.' }, { status: 503 })
