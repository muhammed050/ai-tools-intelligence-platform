...
export default async function Home() {
  ...
  return <main dir={ar ? 'rtl' : 'ltr'}>
    ...
    {guides.length > 0 && <section className="container" style={{ padding: '54px 0 20px' }}><div className="section-head"><div><div className="eyebrow">{ar ? 'المحتوى الإرشادي' : 'Editorial desk'}</div><h2>{ar ? 'أحدث الأدلة' : 'Latest guides'}</h2></div><Link className="btn btn-secondary" href="/blog">{ar ? 'قراءة جميع الأدلة' : 'Read all guides'} <ArrowRight size={15} /></Link></div><div className="tool-grid">{guides.map((guide) => <Link className="card tool-card" href={`/blog/${guide.slug}`} key={guide.slug}><div className="tool-logo"><Search size={20} /></div><div className="eyebrow" style={{ marginTop: 18 }}>{ar ? 'دليل ذكاء اصطناعي' : 'AI Guide'}</div><h3 style={{ margin: '5px 0' }}>{guide.title}</h3><p className="muted" style={{ margin: 0 }}>{guide.excerpt}</p><span style={{ marginTop: 'auto', color: 'var(--brand)', fontSize: 13 }}>{ar ? 'اقرأ الدليل' : 'Read guide'} <ArrowRight size={14} /></span></Link>)}</div></section>}
