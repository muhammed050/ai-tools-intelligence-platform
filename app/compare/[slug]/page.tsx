...
export default async function CompareDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const comparisonUrl = `${siteUrl}/compare/${slugs.join('-vs-')}`
  const schema = {
    '@context': 'https://schema.org',
    '@graph': [
      { '@type': 'WebPage', name: `${title} comparison`, description: `Compare ${title} by pricing, ratings, features, platforms, strengths and limitations.`, url: comparisonUrl, isPartOf: { '@type': 'WebSite', name: 'Eldevo', url: siteUrl } },
      { '@type': 'BreadcrumbList', itemListElement: [
        { '@type': 'ListItem', position: 1, name: 'AI Tools', item: `${siteUrl}/tools` },
        { '@type': 'ListItem', position: 2, name: 'Compare AI Tools', item: `${siteUrl}/compare` },
        { '@type': 'ListItem', position: 3, name: title, item: comparisonUrl },
      ] },
      ...tools.map((tool) => ({ '@type': 'SoftwareApplication', name: tool.name, description: tool.short_description, url: `${siteUrl}/tools/${tool.slug}`, applicationCategory: tool.category?.name || 'AI software', operatingSystem: tool.platforms.length ? tool.platforms.join(', ') : 'Web' })),
    ],
  }
  ...
  const finderQuery = `I am choosing between ${tools.map((tool) => tool.name).join(' and ')}. Help me decide based on my use case, budget, platform and required features.`
  return <main className="container" style={{ padding: '55px 0 100px' }}><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }} /><nav className="muted" aria-label="Breadcrumb"><Link href="/tools">AI Tools</Link> / <Link href="/compare">Compare</Link> / {title}</nav><section className="card hero-glow" style={{ padding: 30, marginTop: 18 }}><div className="tool-logo"><GitCompareArrowsIcon /></div><div className="eyebrow" style={{ marginTop: 16 }}>Database comparison</div><h1 style={{ fontSize: 'clamp(36px,5vw,58px)', letterSpacing: '-.045em', margin: '7px 0 12px' }}>{title}</h1><p className="muted" style={{ fontSize: 18 }}>Compare published platform data side by side. Unknown values are labeled instead of estimated.</p><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}><Link className="btn btn-primary" href={`/ai-finder?q=${encodeURIComponent(finderQuery)}`}>Ask Eldevo which fits me <ArrowRight size={15} /></Link><Link className="btn btn-secondary" href="/best-ai-tools">Explore AI guides <ArrowRight size={15} /></Link></div></section><div style={{ overflowX: 'auto', marginTop: 30 }}><table className="comparison-table"><thead><tr><th scope="col">Criteria</th>{tools.map((tool) => <th scope="col" key={tool.id}><Link href={`/tools/${tool.slug}`}>{tool.name}</Link>{tool.verified && <span className="badge" style={{ marginLeft: 8 }}><CheckCircle2 size={12} /> Verified</span>}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={row.label}><th scope="row">{row.label}</th>{row.values.map((value, index) => <td key={`${row.label}-${tools[index].id}`}>{value}</td>)}</tr>)}</tbody></table></div><section className="comparison-guidance card"><div className="eyebrow">How to choose</div><h2>Use the criteria that matter to your workflow.</h2><p className="muted">Start with the row that reflects your constraint: pricing for budget-sensitive work, platforms for your existing stack, and features or use cases for the job you need to complete. Eldevo leaves unknown fields clearly marked so the decision stays grounded in published data.</p></section><section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 16, marginTop: 30 }}>{tools.map((tool) => <article className="card" style={{ padding: 22 }} key={tool.id}><h2 style={{ marginTop: 0 }}>{tool.name}</h2><p className="muted">{tool.short_description}</p><Link className="btn btn-secondary" href={`/tools/${tool.slug}`}>Read full review <ArrowRight size={15} /></Link></article>)}</section><section className="card" style={{ padding: 24, marginTop: 24 }}><div className="eyebrow"><Sparkles size={14} /> Eldevo decision engine</div><h2>Still not sure?</h2><p className="muted">A comparison shows differences; your best choice depends on the actual job. Let Eldevo apply your budget, platform, required features and constraints before ranking the options.</p><Link className="btn btn-primary" href={`/ai-finder?q=${encodeURIComponent(finderQuery)}`}>Find my best match <ArrowRight size={15} /></Link></section></main>
}

function GitCompareArrowsIcon() { return <span aria-hidden="true">⇄</span> }
