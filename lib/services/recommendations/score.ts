...
function intentRelevance(intent: FinderIntent, tool: ToolRecord, semanticScore = 0, keywordScore = 0) {
  ...
  const corpus = [
    ...
    tool.description,
    ...tool.use_cases,
    ...tool.features.map(f => `${f.name} ${f.slug}`),
    tool.category?.name,
    tool.category?.slug,
  ].filter(Boolean).join(' ');
  const wanted = tokens(requested);
  const available = tokens(corpus);
...
function freshness(tool: ToolRecord) {
  if (!tool.last_verified_at) return .25;
  const days = (Date.now() - new Date(tool.last_verified_at).getTime()) / 86400000;
  return norm(1 - days / 180);
}
...
export function calculateRecommendationScore(intent: FinderIntent, tool: ToolRecord, semanticScore = 0, keywordScore = 0) {
  const score = 100 * (
    intentRelevance(intent, tool, semanticScore, keywordScore) * weights.intent +
    featureFit(intent, tool) * weights.features +
    budgetFit(intent, tool) * weights.budget +
    quality(tool) * weights.quality +
    platformFit(intent, tool) * weights.platform +
    experienceFit(intent, tool) * weights.experience +
    freshness(tool) * weights.freshness +
    reliability(tool) * weights.reliability +
    popularity(tool) * weights.popularity
  );
...
export function explainMatch(intent: FinderIntent, tool: ToolRecord): { why: string[]; limitations: string[] } {
  ...
  const why: string[] = [];
  const matched = tool.features.filter(f => wanted.has(f.slug.toLowerCase()) || wanted.has(f.name.toLowerCase().replace(/\s+/g, '-')));
  matched.slice(0, 3).forEach(f => why.push(`Supports ${f.name}`));
...
export function rankRecommendations(intent: FinderIntent, candidates: Array<{ tool: ToolRecord; semanticScore: number; keywordScore: number }>): Recommendation[] {
  return candidates
    .map(x => ({ tool: x.tool, score: calculateRecommendationScore(intent, x.tool, x.semanticScore, x.keywordScore), semanticScore: x.semanticScore, keywordScore: x.keywordScore, ...explainMatch(intent, x.tool) }))
