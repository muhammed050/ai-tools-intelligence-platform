...
function disallowedByRobots(raw:string){let active=false;for(const line of raw.split(/\r?\n/)){const clean=line.split('#')[0].trim();if(!clean)continue;const [key,value]=clean.split(':',2).map(x=>x.trim().toLowerCase());if(key==='user-agent')active=value==='*';else if(active&&key==='disallow'&&value==='/')return true;}return false;}
export async function importPublicTool(url:string):Promise<ImportedTool>{
 ...
 const robots=new URL('/robots.txt',u.origin); const robotsResponse=await fetch(robots,{cache:'no-store'}); if(robotsResponse.ok&&disallowedByRobots(await robotsResponse.text()))throw new Error('Robots policy disallows automated import');
 const response=await fetch(url,{headers:{'user-agent':'AI-Tools-Intelligence-Platform/1.0 (+public-import)'},redirect:'follow',cache:'no-store',signal:AbortSignal.timeout(15000)}); if(!response.ok)throw new Error(`Source returned ${response.status}`); const html=await response.text(); if(html.length>2_000_000)throw new Error('Source page is too large');
 const plain=text(html).slice(0,30000); const provider=getAIProvider(); if(!provider){const title=html.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1]?.trim();return {name:title||undefined,source_url:url,source_type:'public-page',confidence_score:title?20:0};}
