...
export async function getUser(){const s=await createClient();const{data,error}=await s.auth.getUser();return error?null:data.user}
export async function getProfile(){const user=await getUser();if(!user)return null;const s=await createClient();const{data}=await s.from('profiles').select('*').eq('id',user.id).maybeSingle();return data}
